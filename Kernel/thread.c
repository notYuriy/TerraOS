...
void thread_summon(thread_entry_point_t main, size_t stack_size){
    task_state_t* new_task = kaslub_new(&tasks_stub);
    ...
    new_task->prev = thread_current;
    new_task->next = thread_current->next;
    thread_current->next->prev = new_task;
...
void thread_exit(void){
    thread_preemptable = 0;
    thread_current->prev->next = thread_current->next;
    thread_current->next->prev = thread_current->prev;
    kheap_free_nonblock(thread_current->stack_base);
    kaslub_delete(&tasks_stub, thread_current);
    thread_preemptable = 1;
    while(1) asm("pause":::);
}
...
void thread_init_subsystem(void){
    ...
    kaslub_init(&tasks_stub, sizeof(task_state_t), 64);
    timer_set_callback(thread_scheduler_isr);
    thread_current = kaslub_new(&tasks_stub);
